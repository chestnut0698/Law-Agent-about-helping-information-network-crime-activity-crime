...
PLANS = [
    ...
    [
        ...
        "对卡号、手机号等强标识做跨案比对，并根据材料补写需人判断的疑似同一对象。确认完成前不写线索、不写报告。",
        "从材料中整理转账、联络等事件，按已确认同一的对象归并主体。",
        "对照时间线写入疑似关联线索；核验完成前不写报告。禁止法律结论。",
        "线索全部核验后再撰写核验单，供核对与导出。",
    ],
]
PLAN_ENTITY_REVIEW_INDEX = 1
PLAN_TIMELINE_INDEX = 2
PLAN_CLUE_REVIEW_INDEX = 3
PLAN_REPORT_INDEX = 4
...
# 容器内需监听 0.0.0.0，否则宿主机连不上；本地默认仍只开放回环。
...
- 需要说明动作时，只用：「查阅材料」「跨案标识比对」「写入待核对象」「对照证件页面」「整理事件时间线」「查看关联提示」「形成疑似关联线索」「核对脱敏映射」「请到中间工作区打开相应成果核对原文」。
...
3. 跨案标识比对完成后：
   ...
   - 补写后若仍有待核对象：必须停步并结束本轮回复；
   - 先请用户到中间工作区「材料中心」打开本任务脱敏映射，核对化名、号码是否与材料相符，不妥可自行改映射后再脱敏；
   - 再请用户到「实体复核」对每条作出「视为同一」或「保留独立」；
   - 在人工确认完成前，禁止写入线索或生成报告；
   - 不要反复查询等待，确认完成后系统会再请你继续；
   - 用自然语言说明：请先核对脱敏映射（不妥可自行修改），再到实体复核逐条确认；全部确认后我会自动继续后续分析。
