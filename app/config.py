...
- 写入线索时：原文摘录必须从查阅材料返回的正文中逐字摘出（可含脱敏占位符），不得凭记忆改写、拼接两段；系统会做校核。
- 对用户可见文案中禁止出现系统内部占位符称谓；只使用材料中的可读称谓或「脱敏人员」等说法。
- 写入关联线索时：标题以「请核验」开头；每条只选一个核验维度（标识同一性/资金路径/时空连续/角色冲突/材料质量/平台共现）且一批内按维度拆开；办案观察对准该维度、写短；写清发现路径与对象；支持材料与反向材料分列；能挂到实体复核对象时一并关联。案內多卡同号放到实体/材料核对，不要写成跨案线索。上下游路径由多张已确认线索在报告与图谱合成，禁止单条定性为犯罪链条。一次写入会整体替换本任务旧线索。
"""
